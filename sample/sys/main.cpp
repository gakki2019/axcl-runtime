...
static int sample_sys_commom_pool() {
    ...
    if (AXCL_SUCC != ret) {
        SAMPLE_LOG_E("AXCL_SYS_Init fail!!ret:0x%x", ret);
        return -1;
    } else {
    ...
    if (AXCL_SUCC != ret) {
        SAMPLE_LOG_E("AXCL_POOL_Exit fail!!ret:0x%x", ret);
        goto error0;
    } else {
    ...
    if (AXCL_SUCC != ret) {
        SAMPLE_LOG_E("AXCL_POOL_SetConfig fail!ret:0x%x", ret);
        goto error0;
    } else {
    ...
    ret = AXCL_POOL_Init();
    if (AXCL_SUCC != ret) {
        SAMPLE_LOG_E("AXCL_POOL_Init fail!!ret:0x%x", ret);
        goto error0;
    } else {
    ...
    */
    ...
    if (AXCL_SUCC != ret) {
        SAMPLE_LOG_E("AXCL_POOL_ReleaseBlock fail!ret:0x%x", ret);
        goto error1;
    } else {
    ...
    if (AXCL_SUCC != ret) {
        SAMPLE_LOG_E("AXCL_SYS_Deinit fail!!ret:0x%x", ret);
        goto error1;
    } else {
...
static int sample_sys_private_pool() {
    ...
    if (AXCL_SUCC != ret) {
        SAMPLE_LOG_E("AXCL_POOL_DestroyPool fail!ret:0x%x", ret);
        goto error1;
    } else {

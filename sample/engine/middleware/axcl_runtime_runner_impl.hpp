...
struct middleware::runtime_runner::impl {
    ...
    [[nodiscard]] bool prepare(const bool& input_cached, const bool& output_cached, const uint32_t& group, const uint32_t& batch) {
        ...
        for (uint32_t i = 0; i < input_count; i++) {
            ...
            if (original_size = static_cast<uint32_t>(axclrtEngineGetInputSizeByIndex(this->info_, group, i)); 0 == original_size) {
        ...
        for (uint32_t i = 0; i < output_count; i++) {
            uint32_t original_size = 0;
            if (original_size = static_cast<uint32_t>(axclrtEngineGetOutputSizeByIndex(this->info_, group, i)); 0 == original_size) {
    ...
    [[nodiscard]] uintmax_t get_input_size(const uint32_t& index) const {
        ...
        return static_cast<uintmax_t>(axclrtEngineGetInputSizeByIndex(this->info_, this->group_, index));
    }
    ...
    [[nodiscard]] uintmax_t get_output_size(const uint32_t& index) const {
        ...
        return static_cast<uintmax_t>(axclrtEngineGetOutputSizeByIndex(this->info_, this->group_, index));
    }

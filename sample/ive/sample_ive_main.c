...
AX_VOID SAMPLE_IVE_Usage(AX_CHAR* pchPrgName)
{
    ...
    printf("\t-d | --device_id: Device index from 0 to connected device num - 1, optional \n");
...
int main(int argc, char *argv[])
{
    ...
    if (devID < 0 || devID >= (AX_S32)deviceList.num) {
        SAMPLE_IVE_PRT("device index %d is out of connected device num %d\n", devID, deviceList.num);
        axclFinalize();
        return 1;
    }

    SAMPLE_IVE_PRT("device index: %d, bus number: %d\n", devID, deviceList.devices[devID]);
    devID = deviceList.devices[devID];

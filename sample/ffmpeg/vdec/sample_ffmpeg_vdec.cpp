...
int main(int argc, char *argv[])
{
    ...
    AX_VOID *p_lu = NULL;
    AX_VOID *p_ch = NULL;
    AX_BOOL resize_class = AX_FALSE;
    AVDictionary *codec_opts = NULL;

    std::chrono::steady_clock::time_point time_begin;
    std::chrono::steady_clock::time_point time_end;
    std::chrono::microseconds total_usec;
    AX_U32 total_usec_f = 0;
    float total_msec_f = 0.0f;
    float msec_per_f = 0.0f;
...
#ifdef WINDOWS
    parser.add<std::string>("codec", 'c', "codec name", false, "h264_axdec");
    parser.add<std::string>("input", 'i', "input file path", true);
    parser.add<int>("write", 'y', "write frames flag", false, 0);
    parser.add<std::string>("resize", 'r', "resize resolution", false, "1280x720");
    parser.add<std::string>("device", 'd', "device index", false, "0");
    parser.add<int>("verbose", 'v', "log level", false, 0x1000);

    parser.parse_check(argc, argv);

    std::string codec_name = parser.get<std::string>("codec");
    std::string input_file = parser.get<std::string>("input");
    gWriteFrames = parser.get<int>("write");
    std::string resize_str = parser.get<std::string>("resize");
    std::string device_str = parser.get<std::string>("device");
    log_level = parser.get<int>("verbose");

    strncpy(codec_names, codec_name.c_str(), sizeof(codec_names) - 1);
    codec_names[sizeof(codec_names) - 1] = '\0';

    strncpy(ps8StreamFile, input_file.c_str(), sizeof(ps8StreamFile) - 1);
    ps8StreamFile[sizeof(ps8StreamFile) - 1] = '\0';

    strncpy(resize, resize_str.c_str(), sizeof(resize) - 1);
    resize[sizeof(resize) - 1] = '\0';

    strncpy(device_index, device_str.c_str(), sizeof(device_index) - 1);
    device_index[sizeof(device_index) - 1] = '\0';

    resize_class = parser.exist("resize") ? AX_TRUE : AX_FALSE;
    ...
    while ((c = getopt(argc, argv, "c:i:y:r:d:v:h")) != -1) {
        ...
        case 'r':
            ...
            resize_class = AX_TRUE;
            break;
        case 'd':
            slen = strlen(optarg);
            len = snprintf(device_index, slen + 1, "%s", optarg);
            if ((len < 0) || (len != (int)slen)) {
                printf("snprintf FAILED, len:%d optarg:%s slen:%zu\n",
                    len, optarg, slen);
            }
            break;
        case 'v':
    ...
    for (i = 0; i < (int)pstAvFmtCtx->nb_streams; i++) {
        ...
        while (ret >= 0) {
            ...
            if (gWriteFrames) {
                ...
                for (i = 0; i < frame->height; i++) {
                    ...
                    p_lu = (AX_U8*)p_lu + frame->linesize[0];
                }

                p_ch = frame->data[1];
                for (i = 0; i < frame->height / 2; i++) {
                    fwrite(p_ch, 1, frame->width, fp_out);
                    p_ch = (AX_U8*)p_ch + frame->linesize[1];
                }
                fflush(fp_out);
            }
    ...
    total_usec = std::chrono::duration_cast<std::chrono::microseconds>(time_end - time_begin);
    total_usec_f = (AX_U32)total_usec.count();
    total_msec_f = (float)total_usec_f / 1000.f;
    if (!frame_num) {
        SAMPLE_ERR_LOG("no frame\n");
        goto ERR_RET5;
    }

    msec_per_f = total_msec_f / (float)frame_num;

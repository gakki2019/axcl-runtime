...
int main(int argc, char *argv[]) {
    ...
    for (int32_t i = 0; i < count; ++i) {
        ...
        if (0 != ffmpeg_create_demuxer(&demuxers[i], url.c_str(), grp, device_id, {on_receive_demux_stream_data}, 0, mem_stream)) {
...
static void sample_get_decoded_image_thread(AX_VDEC_GRP grp, int32_t device_id, axcl::event *eof_event, SAMPLE_VDEC_CHN_INFO chn_info,
    ...
    if (dump) {
        if (size <= 0) {
            ...
            SAMPLE_LOG_E("[decoder %2d] decode nv12 frame size = %zu", grp, size);
            return;
        }

        if (!allocator.alloc(size)) {
            SAMPLE_LOG_E("[decoder %2d] alloc cma mem size %zu fail", grp, size);
            axclrtDestroyContext(context);
            return;
        }

        SAMPLE_LOG_I("[decoder %2d] alloc cma mem size %zu", grp, size);
    }
    ...
    SAMPLE_LOG_I("[decoder %2d] total decode %llu frames", grp, count);
...
static void on_receive_demux_stream_data(const struct stream_data *data, uint64_t userdata) {
    ...
    if (0 != ret && !g_exit) {
        SAMPLE_LOG_E("[decoder %2d] send stream (id: %lu, size: %u) fail, ret = 0x%x", grp, (unsigned long)data->video.seq_num, data->video.size, ret);
    }
}
